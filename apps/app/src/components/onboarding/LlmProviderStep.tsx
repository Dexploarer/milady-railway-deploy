...
import { useApp, getVrmUrl, getVrmPreviewUrl } from "../../AppContext";
import { client } from "../../api-client";
import { formatRequestError } from "../../utils/error";
import { OnboardingVrmAvatar } from "./OnboardingVrmAvatar";
import type { ProviderOption, PiAiModelOption, OpenRouterModelOption } from "../../api-client";
...
export function LlmProviderStep() {
    ...
    const subscriptionProviders = providers.filter(
        (p: ProviderOption) => p.id === "anthropic-subscription" || p.id === "openai-subscription"
    );
    ...
    if (!onboardingProvider) {
        return (
            <div className="w-full mx-auto mt-10 text-center font-body">
                <OnboardingVrmAvatar vrmPath={avatarVrmPath} fallbackPreviewUrl={avatarFallbackPreviewUrl} />
    ...
    return (
        <div className="max-w-[520px] mx-auto mt-10 text-center font-body">
            ...
            {onboardingProvider === "elizacloud" && (
                <div className="max-w-[600px] mx-auto text-left">
                    ...
                    {onboardingElizaCloudTab === "login" ? (
                        ...
                        <div>
                            <label htmlFor="elizacloud-apikey" className="block text-sm text-txt mb-1.5">
                ...
                <div className="text-left">
                    ...
                    {onboardingSubscriptionTab === "token" ? (
                        ...
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-xs text-muted text-center">{t("onboardingwizard.YourClaudeSubscrip")}</p>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-xs text-muted text-center">{t("onboardingwizard.RequiresClaudePro")}</p>
                            {anthropicError && <p className="text-xs text-red-400">{anthropicError}</p>}
                        </div>
                    ) : (
                        <div className="flex flex-col items-center gap-3">
                            <p className="text-sm text-txt text-center">
                                {t("onboardingwizard.AfterLoggingInYo")}<br />{t("onboardingwizard.CopyAndPasteItBe")}
                            </p>
                            <input
                                type="text"
                ...
                <div className="space-y-4">
                    {openaiConnected ? (
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-xs text-muted text-center">{t("onboardingwizard.YourChatGPTSubscri")}</p>
                        </div>
                    ) : !openaiOAuthStarted ? (
                        <div className="flex flex-col items-center gap-3">
                            <button
                            ...
                            <p className="text-xs text-muted text-center">{t("onboardingwizard.RequiresChatGPTPlu")}</p>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-3">
                            <div className="p-3 border border-border bg-card text-sm text-fg rounded">
                                ...
                                <p className="text-muted text-xs leading-relaxed">
                                    ...
                                    <code className="text-fg bg-input px-1 py-0.5 text-xs">{t("onboardingwizard.localhost1455")}</code>
            ...
            {onboardingProvider === "ollama" && (
                <p className="text-xs text-muted">{t("onboardingwizard.NoConfigurationNee")}</p>
            )}
                        ...
                        <div className="flex flex-col gap-2">
                            {onboardingOptions?.openrouterModels?.map((model: OpenRouterModelOption) => (
                                ...
                                >
                                    ...
                                    {model.description && (
                                        <div className="text-xs text-muted mt-0.5">{model.description}</div>
                                    )}
                                </button>
                            ))}
                        </div>
